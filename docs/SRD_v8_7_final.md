...
**Status**: Frozen for implementation. Supersedes SRD v8.6.
...
**Document precedence**: v8.7 > v8.6. Where the two conflict, v8.7 governs.
...
6. **Two-layer PIT protocol.** (a) NFCI family, STLFSI4, MIDAS challenger series are excluded from the production feature set. (b) The production backtest window is split into pseudo-PIT (2001–2011) and strict-PIT (2012-onward); only the strict-PIT segment enters acceptance statistics.
...
- **2012-01-06 → today**: `vintage_mode="strict"`. All §16 acceptance thresholds are computed on this segment only.
...
## 16. Acceptance tests (gated on strict-PIT segment 2012→ only)

A coding agent's implementation passes v8.7 iff every item below passes.
...
### 16.2 Statistical (strict-PIT segment 2012→)

Numerical thresholds, frozen:

6. **Interior coverage**:
   - `|empirical_coverage(q10) − 0.10| ≤ 0.03` over the full strict-PIT segment
   - `|empirical_coverage(q90) − 0.90| ≤ 0.03` over the full strict-PIT segment
...
11. **Safety**:
    - zero `NaN` or `Inf` in production output over the full segment
